...
export class MessageService {
  ...
  async sendMessageToGroup(createMessageInput: CreateMessageInput) {
    const message = new this.messageModel({...createMessageInput, receiver_model: 'Group'});
    await message.save()
    return this.groupModel.findByIdAndUpdate(
      createMessageInput.receiver,
      { $push: { messages: message._id } },
      { new: true, useFindAndModify: false }
    ).populate('members').populate('messages').lean()
  }

  async sendCommentToPost(createMessageInput: CreateMessageInput) {
    const message = new this.messageModel({...createMessageInput, receiver_model: 'Post'});
    await message.save()
    return this.postModel.findByIdAndUpdate(
      createMessageInput.receiver,
      { $push: { comments: message._id } },
      { new: true, useFindAndModify: false }
    ).populate('comments').populate('channel').lean()
  }

  async sendMessageToChat(createMessageInput: CreateMessageInput) {
    const message = new this.messageModel({...createMessageInput, receiver_model: 'Chat'});
    await message.save()
    return this.chatModel.findByIdAndUpdate(
      createMessageInput.receiver,
      { $push: { messages: message._id } },
      { new: true, useFindAndModify: false }
    ).populate('messages').populate('first_companion').populate('second_companion').lean()
  }
  ...
  async updateMessageById(updateMessageInput: UpdateMessageInput) {
    return this.messageModel.findByIdAndUpdate(
      ...
      { new: true, useFindAndModify: false }
    ).populate('sender').populate('receiver').lean()
  }

  async deleteMessageById(id: string) {
    return this.messageModel.findByIdAndDelete(id).populate('sender').populate('receiver').lean()
  }
}
